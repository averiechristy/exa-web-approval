...
<script>
    ...
    pdfArea.addEventListener('drop', function (e) {
        ...
        if (currentFile.usedSigners.has(draggedSigner.id)) return;
        ...
        currentFile.signatures.push({
            ...
            tier: parseInt(box.dataset.tier),
            page: currentPage,
            x_percent: xPercent,
            y_percent: yPercent
        });
        ...
        files.forEach(file => {
            ...
            reader.onload = function () {
                ...
                pdfjsLib.getDocument(pdfData).promise.then(pdf => {
                    ...
                    // Otomatis tampilkan file pertama
    ...
    function renderSignaturesForPage(page) {
        ...
        // Hanya render visual jika Custom mode
        file.signatures.forEach(sig => {
            if (sig.page === page) {
                const box = document.createElement('div');
                box.classList.add('signature-box');
                box.dataset.signerId = sig.signer_id || sig.signer;
                box.dataset.signerName = sig.signer_name || sig.signer || 'Unknown';
                ...
                box.innerHTML = `
                    <div class="delete-signature">&times;</div>
                    <div class="signer-info">
                        <span class="signer-name">${box.dataset.signerName}</span><br>
                        <small class="tier-info">Tier ${box.dataset.tier} • Signature</small>
                    </div>
                `;
    ...
    function collectSignatureData() {
        ...
        return uploadedFiles.map(file => {
            ...
            if(type === 'custom') {
                ...
                const summaryPage = file.totalPages + 1; // Tambah halaman summary
    ...
    function applyStandardFixedSignatures(mode) {
        ...
        uploadedFiles.forEach(file => {
            ...
            if (mode === 'standard') {
                ...
                const lastPage = file.totalPages; // atau +1 jika mau tambah halaman summary
                showOnDocApprovers.forEach((approver, index) => {
                    const positions = [
        ...
        // ✅ PENTING: JANGAN render visual box untuk standard & fixed
        renderSignaturesForPage(currentPage);
    }
    ...
    function collectCompletePayload() {
        ...
        const signatureData = collectSignatureData(); // sudah ada
        ...
        const payload = {
            ...
            document: {
                ...
                document_name: step1.files && step1.files.length > 0 ? step1.files[0] : null, // atau ambil dari input jika ada
                requester_id: null, // backend ambil dari auth
                requester_division_id: step1.division_id,
                workflow_id: parseInt(step1.document_type_id), // workflow_id = document_type_id
                status: 'DRAFT'
            },

            // Files (base64 atau handle di backend)
            files: uploadedFiles.map(file => ({
                name: file.name,
                content: Array.from(file.pdfData) // atau kirim sebagai FormData nanti
            })),
            ...
            document_shares: step2.cc_users.map(cc => ({
                share_to: cc.user_id,
                share_by: null // backend ambil dari auth
            })),

            // Email
            email_subject: "Dokumen untuk Persetujuan - " + (step1.files && step1.files[0] ? step1.files[0] : "Untitled"),
            email_message: "Mohon review dan berikan persetujuan atas dokumen terlampir.",

            // Metadata tambahan
            placement_type: document.querySelector('input[name="placementType"]:checked')?.value || 'custom'
        };

        // ================= BUILD document_approvals + approval_positions =================
        let approvalIdCounter = 1; // simulasi (backend akan generate)

        step2.approvers.forEach(tierData => {
            tierData.approvers.forEach((approver, idx) => {
                const approval = {
                    id: approvalIdCounter++, // temporary
                    ...
                    status: 'PENDING',
                    tier: tierData.tier,
                    // workflow_step_id: bisa diambil dari response loadWorkflowApprovers jika ada
                };

                payload.document_approvals.push(approval);

                // Cari posisi signature untuk approver ini
                const positions = [];
                
                signatureData.forEach(fileData => {
                    fileData.signatures
                        .filter(sig => parseInt(sig.approver_id) === parseInt(approver.user_id))
                        .forEach(sig => {
                            positions.push({
                                document_approval_id: approval.id,
                                page_number: sig.page_number,
                                pos_x_percent: parseFloat(sig.pos_x_percent),
                                pos_y_percent: parseFloat(sig.pos_y_percent)
                            });
                        });
                });

                payload.approval_positions = payload.approval_positions.concat(positions);
            });
        });

        console.log('🚀 COMPLETE PAYLOAD TO BACKEND:', payload);
        console.table(payload.document_approvals);
        console.table(payload.approval_positions);
    ...
    document.getElementById('btnSendDocument').addEventListener('click', function () {
        ...
        const payload = collectCompletePayload();

        // Update email dari input Step 4
